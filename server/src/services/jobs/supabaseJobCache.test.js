...
test("builds a stable cache key from provider, role, and location", () => {
  assert.equal(
    ...
    "auto|job-requirements-v1|data analyst intern|sabah, malaysia",
  );
});
...
test("defaults direct cache keys to current Jooble requirement version", () => {
  assert.equal(
    ...
    "jooble|job-requirements-v2|security|malaysia",
  );
});
...
test("defaults Supabase job cache keys to Jooble when no provider override is configured", async () => {
  ...
  assert.equal(result.cacheKey, "jooble|job-requirements-v2|azure devops engineer|malaysia");
  assert.match(calls[0].url, /cache_key=eq\.jooble%7Cjob-requirements-v2%7Cazure\+devops\+engineer%7Cmalaysia/);
});
...
test("loads a valid cached Supabase job search result", async () => {
  ...
  const cache = createSupabaseJobCache({
    ...
    fetchImpl: async (url, options) => {
      ...
      return jsonResponse([
        {
          cache_key: "auto|job-requirements-v2|data analyst|sabah, malaysia",
  ...
  assert.equal(result.cacheKey, "auto|job-requirements-v2|data analyst|sabah, malaysia");
  ...
  assert.match(calls[0].url, /cache_key=eq\.auto%7Cjob-requirements-v2%7Cdata\+analyst%7Csabah%2C\+malaysia/);
...
test("stores job search results in Supabase with an expiry", async () => {
  ...
  const cache = createSupabaseJobCache({
    ...
    fetchImpl: async (url, options) => {
      ...
      return jsonResponse([
        {
          cache_key: "jooble|job-requirements-v2|data analyst|malaysia",
  ...
  assert.equal(result.cacheKey, "jooble|job-requirements-v2|data analyst|malaysia");
  ...
  assert.equal(body.cache_key, "jooble|job-requirements-v2|data analyst|malaysia");
