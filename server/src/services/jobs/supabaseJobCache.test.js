...
test("builds a stable cache key from provider, role, and location", () => {
  assert.equal(
    ...
    "auto|data analyst intern|sabah, malaysia",
  );
});
...
test("loads a valid cached Supabase job search result", async () => {
  ...
  const cache = createSupabaseJobCache({
    ...
    fetchImpl: async (url, options) => {
      ...
      return jsonResponse([
        {
          cache_key: "auto|data analyst|sabah, malaysia",
  ...
  assert.equal(result.cacheKey, "auto|data analyst|sabah, malaysia");
  ...
  assert.match(calls[0].url, /cache_key=eq\.auto%7Cdata\+analyst%7Csabah%2C\+malaysia/);
...
test("stores job search results in Supabase with an expiry", async () => {
  ...
  const cache = createSupabaseJobCache({
    ...
    fetchImpl: async (url, options) => {
      ...
      return jsonResponse([
        {
          cache_key: "jooble|data analyst|malaysia",
  ...
  assert.equal(result.cacheKey, "jooble|data analyst|malaysia");
  ...
  assert.equal(body.cache_key, "jooble|data analyst|malaysia");
